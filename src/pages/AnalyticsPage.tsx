...
import { BarChart3, PieChartIcon, TrendingUp, TrendingDown, DollarSign, Wallet, ArrowUpRight, ArrowDownRight, Calendar, Download, Filter, } from "lucide-react"
import { getAnalyticsSummary, getMonthlyAnalytics, getCategoryAnalytics, } from "../services/analytics"
import { type AnalyticsSummary, type MonthlyAnalytics, type CategoryAnalytics, } from "../services/analytics"
...
export default function AnalyticsPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true)
        const [summaryData, monthlyDataResult, categoryDataResult] = await Promise.all([
          getAnalyticsSummary(),
          getMonthlyAnalytics(),
        ...
        setSummary({
          ...summaryData,
          savingsRate: Number(summaryData.savingsRate) || 0,
        })
        setMonthlyData(monthlyDataResult)
        setCategoryData(categoryDataResult)
      } catch (err) {
        console.error("[v0] Failed to fetch analytics data:", err)
